...
while(cap.isOpened()):
    ...
    #            x,y,w,h = cv2.boundingRect(mask)
                ...
                if(mean_val[0]>mean_val[1] and (mean_val[0]>mean_val[2])):
                    cv2.putText(frame,'TRI_B',(x,y),cv2.FONT_HERSHEY_SIMPLEX,2,(255,255,0),2,cv2.LINE_AA)
                elif(mean_val[2]>mean_val[1] and (mean_val[2]>mean_val[0])):
                    cv2.putText(frame,'TRI_R',(x,y),cv2.FONT_HERSHEY_SIMPLEX,2,(255,255,0),2,cv2.LINE_AA)
                else:
                    cv2.putText(frame,'TRI_Y',(x,y),cv2.FONT_HERSHEY_SIMPLEX,2,(255,255,0),2,cv2.LINE_AA)
                ...
                mean_val = cv2.mean(contours[i]) # the means of the blue, green, and red channels, respectively            
                if((mean_val[0]>mean_val[1]) and (mean_val[0]>mean_val[2])):
                    cv2.putText(frame,'REC_B',(x,y),cv2.FONT_HERSHEY_SIMPLEX,2,(255,255,0),2,cv2.LINE_AA)
                elif((mean_val[2]>mean_val[1]) or (mean_val[2]>mean_val[0])):
                    cv2.putText(frame,'REC_R',(x,y),cv2.FONT_HERSHEY_SIMPLEX,2,(255,255,0),2,cv2.LINE_AA)
                else:
                    cv2.putText(frame,'REC_Y',(x,y),cv2.FONT_HERSHEY_SIMPLEX,2,(255,255,0),2,cv2.LINE_AA)
            else:
                print('None')
        ...
        # press 'q' to exit
        if k == ord('q'):
            factor_contrast=float(input('Enter the Contrast value '))
            factor_brightness = float(input('Enter the Brightness value '))
