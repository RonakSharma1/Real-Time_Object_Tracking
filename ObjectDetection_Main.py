#THIS IS THE ORIGINAL CODE WRITTEN IN THE FIRST PLACE. IT DETECTS THE COLOUR FIRST
# AND THEN THE SHAPE. THIS WILL DETECT AND IGNORE ANY CONTOURS THAT ARE LESS THAN 
# A CERTAIN VALUE.#
import cv2
import numpy as np
...
def shapeIdentifier(median_value,name_tri,name_rectangle):
        ...
        for i in range(0,len(contours)):
            ...
            if(len(approx)<=3):
                ...
                #Rectangle
                if(vtc==4):
                    cv2.putText(frame,name_rectangle,(x,y),cv2.FONT_HERSHEY_SIMPLEX,scale,(255,255,0),2,cv2.LINE_AA)
                    cv2.rectangle(frame,(x,y),(x+w,y+h),(255,0,0),2)
                    cv2.imshow('edges',edges)
            else:
                None
...
#        h=int(input('Enter Hue of Blue '))
#        s =int(input('Enter Saturation of Blue '))
#        v =int(input('Enter Value of Blue '))
